...
class PPSI262:
    ...
    def __init__(self,
        ...
        self.last_fifo_read = time.time_ns()
        ...
        self.register_set(NUMAV, NUMAV_MASK,
                          int(self.samples_to_average - 1))  # Set the ADC output to be the average of [self.samples_to_average] samples

        # Setup LED current drivers and switching
        self.register_set(ILED_FS, ILED_FS_MASK,
                          1)  # Set the full-scale output of the LED current driver to 100mA (set to 0 for 50mA)
        ...
        self.register_set(REG_FIFO_PERIOD, REG_FIFO_PERIOD_MASK,
                          self.sample_depth)  # retain [self.sample_depth] sample cycles
        ...
        _convert_width = int(
            (56.5 * self.samples_to_average + 72) / _tick_period) + 16  # and especially this one is too
    ...
    def read_sensor(self):
        ...
        self.reading_average = (self.reading_average * self.smoothing * 6 + _sensor_output[0][0]) / (
                    self.smoothing * 6 + 1)
